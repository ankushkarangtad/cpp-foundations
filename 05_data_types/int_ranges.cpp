...
int main(){
    ...
    std::cout<<"Size of x(unsigned short) is: "<<sizeof(x)<<" Bytes\n"<<"x (max) = "<<x <<'\n';

    x = x + 1;
    std::cout<<"Adding 1 to x\nNow, x = "<< x <<" Error: Modulo Wrap-around\n"
            <<"---------------------------------------------------------\n\n"; 
    ...
    y = y + 1;
    std::cout<<"Adding 1 to y\nNow, y = "<< y<<" Error: undefined behaviour/ minimum value\n"
            <<"---------------------------------------------------------\n"; 
